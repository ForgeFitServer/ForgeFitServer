...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onBumpAll, onAddSet, onRemoveSet }) {
    ...
    if (col.f === 'rpe') {
      return (
        <div className={'stp ' + cls}>
          <button aria-label="Decrease" onClick={() => onField(i, 'rpe', Math.max(1, (s.rpe ?? 5) - col.step))}><Icon name="minus" /></button>
          <span className="val"><NumberField decimal={col.dec} value={s.rpe ?? ''} onChange={v => onField(i, 'rpe', Math.max(1, Math.min(10, v)))} /></span>
          <button aria-label="Increase" onClick={() => onField(i, 'rpe', Math.min(10, (s.rpe ?? 5) + col.step))}><Icon name="plus" /></button>
        </div>
      )
    }
    return (
  ...
  return <>
    ...
    <div className="card" style={{ marginTop: 10, marginBottom: 0 }}>
      <div className="sethead"><span className="n-sp" /><span className="w-sp">{col1.hd}</span><span className="r-sp">{col2.hd}</span>{col3 && <span className="r-sp">{col3.hd}</span>}<span className="ck-sp" /></div>
      {entry.sets.map((s, i) => <div key={i} className={'setrow' + (s.done ? ' done' : '')}>
        <div className="n">{i + 1}</div>
        {cell(s, i, col1, 'w')}
        {cell(s, i, col2, 'r')}
        {col3 && cell(s, i, col3, 'r')}
...
function ActiveWorkout() {
  ...
  // Auto-save weight when exercise completes (skip the TopWeight modal)
  const autoSaveWeight = idx => {
    const entry = A.entries[idx]
    const maxSet = Math.max(0, ...entry.sets.filter(s => s.done).map(s => s.w || 0))
    const maxWeight = Math.round((maxSet || 0) * 10) / 10
    update(s => {
      s.active.entries[idx].topW = maxWeight
      const cur = s.exWeights[entry.id]
      s.exWeights[entry.id] = { w: Math.max(maxWeight, cur ? cur.w : 0), d: todayISO() }
    })
    
    // Check if this was the last unit
    const unitDone = unit.every(ui => (ui === idx ? A.entries[idx] : A.entries[ui]).sets.every(s => s.done))
    const isLastUnit = unitIdx >= units.length - 1
    
    // Auto-advance to next exercise or show finish prompt
    if (unitDone) {
      if (isLastUnit) {
  ...
  const toggle = (idx, i) => {
    ...
    // Auto-save weight for non-cardio exercises (skip the TopWeight modal).
    // Cardio or already-confirmed: go straight to the prompt.
    if (askTop) autoSaveWeight(idx)
