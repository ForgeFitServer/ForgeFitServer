// Lightweight i18n: English keys, lazy-loaded locale & instruction packs
...
let lang = 'en'
let dict = {}
let instr = null            // { exId: [steps] }, null = English (fallback)
let version = 0
const subs = new Set()
...
// Translate source string; {0},{1}… replaced with args
export function t(s, ...args) {
  let v = dict[s] || s
  for (let i = 0; i < args.length; i++) v = v.replaceAll('{' + i + '}', args[i])
  return v
}
// Instructions for an exercise in current language (English steps as fallback)
