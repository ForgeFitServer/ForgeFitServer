...
export const DEF = {
  ...
  theme: 'dark', accent: 'lime', body: 'male', targetW: null, avatar: null, height: null,
...
export const useStore = create((set, get) => {
  ...
  return {
    ...
    async boot() {
      ...
      // Demo build (GitHub Pages): no backend at all — seed a fresh example profile each
      // session (sessionStorage), so a visitor's changes never persist past the browser.
      if (DEMO) {
        if (!STORAGE.getItem(DEMO_SEEDED)) {
          STORAGE.setItem(DEMO_SEEDED, '1')
          await get().resetDemo()
        }
        get().setGuest(true)
        set({ ready: true })
