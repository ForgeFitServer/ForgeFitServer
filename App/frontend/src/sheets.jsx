...
import { submitQuestionnaire, createQuestionnaire, updateQuestionnaire } from './lib/api.js'
...
/* == starter plan (⅃ƎVEL style) == */
export function loadStarterPlan() {
  const [push, pull, legs] = starterRoutines()
  update(st => {
    st.routines.push(push, pull, legs)
    const prog = {
      id: uid(), name: '⅃ƎVEL - Class: Fighter', emoji: 'forge', coach: 'System',
      desc: 'Deep within the iron-bound halls of the Fighter\'s Guild, legends are not born — they are forged. This plan is crafted for champions of combat; with dumbbells as your weapons and determination as your shield, you will embody the strength, endurance, and versatility of the mighty Fighter class.',
      photo: null, dayIds: [push.id, pull.id, legs.id]
    }
    ;(st.programs = st.programs || []).push(prog)
    st.week[1] = push.id; st.week[3] = pull.id; st.week[5] = legs.id
  })
  toast(t('⅃ƎVEL starter plan loaded — Mon Push · Wed Pull · Fri Legs'))
}
...
function ExerciseDetail({ ex, close }) {
  ...
  return <>
    ...
    {ex.url && <div className="small" style={{ marginBottom: 10, color: 'var(--acc)' }}><a href={ex.url} target="_blank" rel="noopener">{t('Video guide')} →</a></div>}
...
function CustomExForm({ existing, prefill, onDone, close }) {
  ...
  const save = () => {
    ...
    const dup = allExercises(S()).find(e => e.n && e.n.toLowerCase() === name.toLowerCase() && e.id !== (existing || {}).id)
    if (dup) { toast(t('“{0}” already exists', dup.n)); return }
    const d = desc.trim().slice(0, 1000)
    const u = url.trim()
    let id = existing && existing.id
    if (existing) update(s => { const c = (s.customEx || []).find(x => x.id === id); if (c) { c.n = name; c.bp = bp; c.desc = d; if (u) c.url = u; else delete c.url } })
    else {
      id = 'c' + uid()
      const ex = { id, n: name, bp, desc: d, tg: '', eq: 'custom', custom: true }
      if (u) ex.url = u
      update(s => { (s.customEx = s.customEx || []).push(ex) })
    }
    close()
  ...
  return <>
    ...
    <div style={{ height: 10 }} />
    <input className="input" placeholder={t('Video URL (optional) — link to a form video')} value={url} onChange={e => setUrl(e.target.value)} />
    <div style={{ height: 14 }} />
...
function ExercisePicker({ onPick, close }) {
  ...
  let base = all.filter(e =>
    ...
    (!ql || (e.n && e.n.toLowerCase().includes(ql)) || (e.tg || '').includes(ql) || (e.eq || '').includes(ql) || (e.desc || '').toLowerCase().includes(ql)))
...
function ExConfig({ ex, existing, onSave, onDelete, close }) {
  ...
  const save = () => {
    ...
    else onSave({ sets: Math.max(1, Math.round(c.sets) || 3), reps: Math.max(1, Math.round(c.reps) || 10), weight: Math.max(0, c.weight || 0), trackRpe: c.trackRpe || false, url: c.url || undefined })
  }
  return <>
    <h3 className="capitalize">{ex.n}</h3>
    <Media ex={ex} />
      ...
      {cardio ? <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>
            <input type="checkbox" checked={c.trackRpe || false} onChange={e => setC(x => ({ ...x, trackRpe: e.target.checked }))} style={{ cursor: 'pointer', width: 16, height: 16 }} />
            {t('Track RPE')}
          </label>
        </div>
      </>}
...
function DayAssign({ day, close }) {
  ...
  const getProgramForRoutine = (routineId) => (st.programs || []).find(p => (p.dayIds || []).includes(routineId))
  const routinesByProgram = {}
  const unassignedRoutines = []
  
  st.routines.forEach(r => {
    const prog = getProgramForRoutine(r.id)
    if (prog) {
      if (!routinesByProgram[prog.id]) routinesByProgram[prog.id] = { prog, routines: [] }
      routinesByProgram[prog.id].routines.push(r)
    } else {
  ...
  return <>
    ...
    <div className="list">
      ...
      {/* Group by program */}
      {Object.values(routinesByProgram).map(({ prog, routines }) => <div key={prog.id}>
        <div className="item" style={{ paddingLeft: 12, background: 'var(--surface-3)', fontWeight: 600, cursor: 'default' }}>
          <span className="lrow-i"><Icon name={glyphOf(prog.emoji)} /></span>
          <div className="grow"><div className="tt">{prog.name}</div></div>
        </div>
        {routines.map(r => <div key={r.id} className="item" style={{ paddingLeft: 40 }} onClick={() => set(r.id)}>
          <span className="lrow-i"><Icon name={glyphOf(r.emoji)} /></span>
          <div className="grow"><div className="tt">{r.name}</div><div className="ss">{t('{0} exercises', r.ex.length)}</div></div>
          {st.week[day] === r.id && <Icon name="check" className="accent" />}</div>)}
      </div>)}
      
      {/* Unassigned routines - only show if no programs exist */}
      {unassignedRoutines.length > 0 && Object.keys(routinesByProgram).length === 0 && <>
        <div className="item" style={{ paddingLeft: 12, background: 'var(--surface-3)', fontWeight: 600, cursor: 'default' }}>
          <div className="grow"><div className="tt" style={{ fontSize: '0.85em', color: 'var(--label)' }}>{t('Unassigned')}</div></div>
        </div>
        {unassignedRoutines.map(r => <div key={r.id} className="item" style={{ paddingLeft: 40 }} onClick={() => set(r.id)}>
          <span className="lrow-i"><Icon name={glyphOf(r.emoji)} /></span>
          <div className="grow"><div className="tt">{r.name}</div><div className="ss">{t('{0} exercises', r.ex.length)}</div></div>
          {st.week[day] === r.id && <Icon name="check" className="accent" />}</div>)}
      </>}
...
function QuestionnaireSheet({ assignment, close, onDone }) {
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    try {
      await submitQuestionnaire(assignment.id, responses)
      toast(t('Questionnaire submitted'))
      onDone && onDone()
      close()
    } catch (e) {
  ...
  const allFilled = fields.every(f => f.required ? responses[f.id] !== undefined && responses[f.id] !== '' : true)

  return <div>
    <h3 style={{ marginBottom: 12 }}>{assignment.title}</h3>
    ...
    <Button variant="primary" disabled={submitting || !allFilled} onClick={handleSubmit}>
      {submitting ? t('Submitting...') : t('Submit')}
    </Button>
  </div>
}

export const questionnaireSheet = (assignment, onDone) => ui().openSheet(close => <QuestionnaireSheet assignment={assignment} close={close} onDone={onDone} />)

/* == questionnaire editor (coach) == */
function QuestionnaireEditor({ existing, close, onSaved }) {
  const [title, setTitle] = useState(existing?.title || '')
  const [description, setDescription] = useState(existing?.description || '')
  const [fields, setFields] = useState(existing?.fields ? clone(existing.fields) : [])
  const [saving, setSaving] = useState(false)

  const addField = () => setFields(f => [...f, { id: 'f' + uid(), type: 'text', label: '', required: true }])
  const updateField = (i, patch) => setFields(f => f.map((x, j) => j === i ? { ...x, ...patch } : x))
  const removeField = i => setFields(f => f.filter((_, j) => j !== i))
  const moveField = (i, dir) => setFields(f => {
    const j = i + dir; if (j < 0 || j >= f.length) return f
    const n = [...f];[n[i], n[j]] = [n[j], n[i]]; return n
  })

  const save = async () => {
    const ttl = title.trim()
    if (!ttl) { toast(t('Enter a title')); return }
    const clean = fields.map(f => ({ ...f, label: (f.label || '').trim() })).filter(f => f.label)
    setSaving(true)
    try {
      if (existing) await updateQuestionnaire(existing.id, ttl, description.trim(), clean)
      else await createQuestionnaire(ttl, description.trim(), clean)
      toast(existing ? t('Questionnaire updated') : t('Questionnaire created'))
      onSaved && onSaved()
      close()
  ...
  return <div>
    <h3 style={{ marginBottom: 12 }}>{existing ? t('Edit questionnaire') : t('New questionnaire')}</h3>
    <input className="input" placeholder={t('Title')} maxLength={120} value={title} onChange={e => setTitle(e.target.value)} />
    <div style={{ height: 8 }} />
    <input className="input" placeholder={t('Description (optional)')} maxLength={500} value={description} onChange={e => setDescription(e.target.value)} />

    <h4 className="sec" style={{ marginTop: 18 }}>{t('Fields')}</h4>
    {fields.length === 0 && <div className="empty" style={{ marginBottom: 8 }}>{t('No fields yet')}</div>}
    <div className="list" style={{ gap: 10 }}>
      {fields.map((f, i) => <div key={f.id} className="card" style={{ margin: 0, padding: 12 }}>
        <div className="row" style={{ gap: 8, marginBottom: 8 }}>
          <input className="input" placeholder={t('Field label')} maxLength={80} value={f.label} onChange={e => updateField(i, { label: e.target.value })} style={{ flex: 1 }} />
          <button className="iconbtn" onClick={() => moveField(i, -1)} disabled={i === 0} aria-label="up"><Icon name="chevronUp" /></button>
          <button className="iconbtn" onClick={() => moveField(i, 1)} disabled={i === fields.length - 1} aria-label="down"><Icon name="chevronDown" /></button>
          <button className="iconbtn" onClick={() => removeField(i)} aria-label="remove" style={{ color: 'var(--red)' }}><Icon name="trash" /></button>
        </div>
        <div className="row between">
          <select className="input" value={f.type} onChange={e => updateField(i, { type: e.target.value })} style={{ width: 140 }}>
          ...
          <label className="row" style={{ gap: 6, cursor: 'pointer' }}>
            ...
            <span className={'sw' + (f.required ? ' on' : '')} onClick={() => updateField(i, { required: !f.required })} />
          </label>
        </div>
      </div>)}
    </div>
    <div style={{ height: 10 }} />
    <Button icon="plus" onClick={addField}>{t('Add field')}</Button>
    <div style={{ height: 14 }} />
    <Button variant="primary" disabled={saving} onClick={save}>{saving ? t('Saving...') : t('Save questionnaire')}</Button>
  </div>
}

export const questionnaireEditorSheet = (existing, onSaved) => ui().openSheet(close => <QuestionnaireEditor existing={existing} close={close} onSaved={onSaved} />)
